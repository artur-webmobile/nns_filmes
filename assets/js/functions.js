...
if (splashGrid.length) {
  splashGrid.imagesLoaded(function() {
    var $splashWrapper = $(".splash-grid").isotope({
      itemSelector: ".splash-item",
      transitionDuration: 300 // 0.3 second
    });

    var filter = $(".portfolio-filter li");

    // Portfolio Filter //
    filter.on("click", function() {
      var filterValue = $(this).attr("data-filter");
      $splashWrapper.isotope({ filter: filterValue });
